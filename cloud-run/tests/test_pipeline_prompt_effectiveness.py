...
FAKE_PROMPT_METRICS = [
    ...
    {"scene_id": 2, "score": 90, "passed": True, "metrics": {}},
]
...
class TestPromptEffectivenessFeatureFlag(unittest.TestCase):
    ...
    def test_flag_off_is_never_called(self):
        with patched_pipeline() as m:
            ...
            m["prompt_effectiveness"].evaluate_scenes.assert_not_called()
            self.assertNotIn("prompt_metrics", result)

    def test_flag_on_calls_evaluate_scenes_with_pre_and_post_enrichment_prompts(self):
        with patched_pipeline() as m, \
            ...
            m["prompt_effectiveness"].evaluate_scenes.assert_called_once_with(
                STYLED_SCENES, ENRICHED_PROMPT_SCENES, FAKE_SCENE_PLAN,
            )
    ...
    def test_exception_does_not_break_pipeline(self):
        with patched_pipeline() as m, \
             patch("app.pipeline.pipeline.config.ENABLE_PROMPT_EFFECTIVENESS", True):
            _wire_defaults(m)
            m["prompt_effectiveness"].evaluate_scenes.side_effect = Exception("effectiveness boom")
