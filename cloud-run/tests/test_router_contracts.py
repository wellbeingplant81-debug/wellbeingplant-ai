...
ENDPOINTS = [
    ...
    # Sprint189 - 단계마다 무엇이 붙는가. 이미 센 것을 붙이기만 하므로
    # 패치할 것이 없다.
    ("GET", "/studio/api/beta-feedback-insights", None, []),
    ...
    # Sprint192 - 지금을 한 장으로. 앞의 두 자리가 낸 답을 담기만
    # 하므로 패치할 것이 없다.
    ("GET", "/studio/api/beta-snapshot", None, []),
    # Sprint193 - 밖으로 내보낼 한 장. 스냅샷이 낸 것만 옮기므로
    # 패치할 것이 없다.
    ("GET", "/studio/api/beta-release-report", None, []),
