...
def plan_video_search_queries(narration: str, image_prompt: str) -> list:
    ...
    Primary는 generate_semantic_primary_query(image_prompt)다
    (Sprint103 - Semantic Query Intelligence. 이전에는
    extract_search_query()의 위치 기반 8단어 절단을 그대로 썼으나,
    image_prompt_rules.py가 카메라 앵글을 문장 맨 앞에 두도록 강제하는
    구조와 결합해 핵심 명사가 잘려나가는 문제가 Sprint102 Root Cause
    분석에서 확인됐다). image_prompt에 매칭되는 카테고리가 있으면 그
    카테고리의 action_query + fallback_queries를 순서대로 이어
    붙인다. 매칭되는 카테고리가 없으면 Primary 하나만 담긴 리스트를
    반환한다(빈 리스트가 되지 않도록 항상 최소 1개 보장). 중복된
    문자열은 제거한다(순서는 유지).
    ...
    primary = generate_semantic_primary_query(image_prompt)
